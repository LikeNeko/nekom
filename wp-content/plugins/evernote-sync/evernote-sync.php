...
class EvernoteSyncLoader
{
                ...
                if (is_array($tagGuids)) {
                    foreach ($tagGuids as $tagGuid) {
                        ...
                        if ($cat_ID > 0) {
                            ...
                            $publishMode = 'private';
                        } else if ($tagName == 'draft') {
                            $publishMode = 'draft';
                        } else if ($tagName == 'publish') {
                            $publishMode = 'publish';
                        } else if ($tagName != $syncTag) {
                            // 拼接标签
                            if (strlen($tags) > 0) {
                                $tags = $tags . ',';
                            }
    ...
    public static function showOptionsPage()
    {
        ...
        // 手动同步
        if (isset($_POST['sync'])) {
            error_log("manual sync");
            EvernoteSyncLoader::sync(true);
            ?>
            <div class="updated"><?php _e('Sync success', 'evernotesync') ?></div>
        <?php
        }
        // 清楚同步记录
        if (isset($_POST['clear'])) {
            ...
            ?>
            <div class="updated"><?php _e('Clear success', 'evernotesync') ?></div>
        <?php
        }
        ?>
        <script>
        ...
        <form action="" method="post">
                ...
                <tr valign="top">
                    ...
                    <td>
                        <select id="evernotesync_platform" name="evernotesync_platform">
                            <option
                                value="1"<?php if (get_option('evernotesync_platform') == 1) echo ' selected'; ?>><?php _e('Yinxiang', 'evernotesync') ?></option>
                            <option
                                value="2"<?php if (get_option('evernotesync_platform') == 2) echo ' selected'; ?>><?php _e('Evernote', 'evernotesync') ?></option>
                        </select>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row">
                        <label for="evernotesync_token">
                            <?php _e('Developer Token', 'evernotesync') ?>
                        </label>
                    </th>
                    <td>
                ...
                <tr valign="top">
                    ...
                    <td>
                        <select id="publishMode" name="evernotesync_publish_mode">
                            <option
                                value="1"<?php if ($mode == 1) echo ' selected'; ?>><?php _e('published', 'evernotesync') ?></option>
                            <option
                                value="2"<?php if ($mode == 2) echo ' selected'; ?>><?php _e('draft', 'evernotesync') ?></option>
                            <option
                                value="3"<?php if ($mode == 3) echo ' selected'; ?>><?php _e('timed', 'evernotesync') ?></option>
                        </select>
                ...
                <tr valign="top">
                    ...
                    <td>
                        <select id="evernotesync_publish_time" name="evernotesync_publish_time">
                            <option
                                value="1"<?php if (empty($publish_time) || $publish_time == 1) echo ' selected'; ?>><?php _e('use WordPress creation time', 'evernotesync') ?></option>
                            <option
                                value="2"<?php if ($publish_time == 2) echo ' selected'; ?>><?php _e('use EverNote creation time', 'evernotesync') ?></option>
                        </select>
                    </td>
                </tr>
                <tr valign="top">
                    ...
                    <td>
                        <select id="evernotesync_format_content" name="evernotesync_format_content">
                            <option
                                value="1"<?php if (empty($format_content) || $format_content == 1) echo ' selected'; ?>><?php _e('Yes', 'evernotesync') ?></option>
                            <option
                                value="2"<?php if ($format_content == 2) echo ' selected'; ?>><?php _e('No', 'evernotesync') ?></option>
                        </select>
                    </td>
                </tr>
                <tr valign="top">
                    ...
                    <td>
                        <select id="evernotesync_generate_excerpt" name="evernotesync_generate_excerpt">
                            <option
                                value="1"<?php if (empty($generate_excerpt) || $generate_excerpt == 1) echo ' selected'; ?>><?php _e('Yes', 'evernotesync') ?></option>
                            <option
                                value="2"<?php if ($generate_excerpt == 2) echo ' selected'; ?>><?php _e('No', 'evernotesync') ?></option>
                        </select>
                    </td>
                </tr>
                <tr valign="top">
            ...
            <p class="submit">
                <input type="submit" name="submit" class="button-primary"
                       value="<?php _e('Save Options', 'evernotesync') ?>"/>
            </p>
...
// 设置定时器
add_action('wp', 'evernote_sync_timer');
