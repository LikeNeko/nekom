...
function poi_player_options_code(){ ?>
  <div id="poi-player-body">
    <div class="poi-player-center">
      ...
      <hr>
      <p>又一款WordPress网易云外链音乐播放器插件</p>
      <p>集成了<code>音乐搜索</code>功能，支持<code>随机播放</code>，<code>进度条拖拉</code>，支持<code>Pjax</code>或<code>ajax站点全站播放</code>。</p>
      <hr>
      ...
      <form method="post" action="options.php">
        ...
        $type = isset($_POST['select'])?$_POST['select']:''; 
	      ...
	      ?></textarea>
	      <div class="poi-other">
	        <input type="checkbox" name="poi_options[autoplay]" class="autoplay" value="0" <?php checked('0',$poi['autoplay']); ?> /><span>自动播放<code>[推荐]</code></span>
          <input type="checkbox" name="poi_options[shuffle]" class="shuffle" value="0" <?php checked('0',$poi['shuffle']); ?> /><span>随机播放<code>[推荐]</code></span>
          <input type="checkbox" name="poi_options[mobilehide]" class="shuffle" value="0" <?php checked('0',$poi['mobilehide']); ?> /><span>移动端不显示<code>[不推荐]</code></span>
            <hr>
          <input type="checkbox" name="poi_options[searchide]" class="shuffle" value="0" <?php //checked('0',$poi['searchide']); ?> /><span>关闭搜索</span>
          <input type="text" name="poi_options[searchnum]" class="searchnum" value="<?php //if($poi['searchnum'] < 100){echo $poi['searchnum'];}elseif($poi['searchnum'] > 100){echo '100';}else{echo '20';} ?>" size="2"><span> 歌曲搜索数量(Max = <code>100</code>）</span>
	      </div>
      ...
      <p>以上两种作为类型的识别链接，自行到网易云音乐挑选歌单或专辑</p>
        <hr>
       <p>外链获取工具</p>
       <p>新建页面添加短代码<code>[PoiMusic]</code></p>
        <hr>
       <p>注意事项</p>
       <p>搜索功能需要主机开启<code>fsockopen()</code>函数，否则无法读取数据，另外可能存在主机IP被墙而无法播放歌曲的问题，大多出现在海外IP的站点。</p>
      <hr>
      <p>Poi Player 版本<?php echo POI_VERSION; ?> 最后更新于<code>2016.12.12</code> <code>@Louie</code></p>
        <hr>
        <p>注释添加者：<code>@Saber</code></p>
