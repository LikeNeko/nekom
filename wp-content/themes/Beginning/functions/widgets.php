...
function Bing_widgets_init(){
	...
	if( $sidebars_list = Bing_mpanel( 'sidebars_list' ) )
	...
	foreach( glob( get_template_directory() . '/includes/widgets/widget-*.php' ) as $file_path )
		include( $file_path );
...
function Bing_current_sidebar(){
	...
	elseif( is_page()     ) $sidebar = Bing_mpanel( 'sidebar_location_page_' . get_the_ID()                );
	...
	if( !empty( $sidebar ) && isset( $GLOBALS['wp_registered_sidebars'][$user_sidebar = sanitize_title( THEME_SLUG . '_user_' . md5( $sidebar ) )] ) )
		return $user_sidebar;
