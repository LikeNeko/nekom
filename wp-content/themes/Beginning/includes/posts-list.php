...
if ( !function_exists( 'Bing_posts_list' ) ) :
	...
	function Bing_posts_list(){
		if ( have_posts() ) :
			...
			Bing_page_navi( array(
				'before' => '<div class="span12 posts-list-page-navi"><div class="panel page-navi">',
				'after' => '</div></div>'
			) );
...
if ( !function_exists( 'Bing_thumbnail' ) ) :
	...
	function Bing_thumbnail( $width, $height = null, $link = true ) {
		...
		if ( Bing_mpanel( 'crop_thumbnail' ) )
			$url = Bing_crop_thumbnail( $url, $width, $height );

		$code = sprintf(
			'<img src="%s" class="thumbnail" width="%s" height="%s" title="%s" alt="%s" />',
			esc_url( $url ),
			esc_attr( $width ),
			esc_attr( $height ),
			$title_attribute,
		...
		if ( $link )
			$code = sprintf(
				'<a href="%s" class="thumbnail-link" rel="bookmark" title="%s">%s</a>',
...
if ( !function_exists( 'Bing_post_meta' ) ) :
	...
	?>
						<li class="views">
							...
							<a href="javascript:;" title="<?php echo esc_attr( sprintf( __( '浏览了%s次', 'Bing' ), Bing_Views_Stats::get() ) ); ?>">
