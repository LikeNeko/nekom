<?php
if ( !function_exists( 'Bing_related_posts' ) ) :
	/**
	 * 相关文章
	 */
	function Bing_related_posts(){
		if( !$related_posts_ID = Bing_related_posts_ID() )
			return;
		...
		if( !$query->have_posts() )
			return;
	?>
		<div class="span12 related-posts-box<?php if ( Bing_mpanel( 'mobile_single_collapse' ) ) echo ' mobile-hide collapse'; ?>">
...
if ( !function_exists( 'Bing_related_posts_loop' ) ) :
	/**
	 * 相关文章样式
	 */
...
function Bing_related_posts_refresh(){
	if( empty( $_REQUEST['post_id'] ) || get_post_status( $_REQUEST['post_id'] ) != 'publish' || !$related_posts_ID = Bing_related_posts_ID( $_REQUEST['post_id'] ) )
		return;
